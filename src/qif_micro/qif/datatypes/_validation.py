...
def _is_dist_valid(
    ...
    for s in dist:
        if np.any((s.data if sp.issparse(s) else s) < 0):
            return _Check(error=_Error.NEGATIVE_VALUES)
    ...
    def reduce_fn(acc, s): return acc + s.sum(axis=dist_orient)
    axis_sum = [s.sum(axis=dist_orient) for s in dist]

    if dist_orient == 0:
         axis_sum = np.hstack(axis_sum)
    else:
         axis_sum = reduce(operator.add, axis_sum)

    sum_is_one = np.isclose(axis_sum, 1)
    sum_exceeds_one = ((axis_sum > 1) & ~sum_is_one).any()

    if sum_exceeds_one:
        return _Check(error=_Error.AXIS_SUM_MISMATCH)
